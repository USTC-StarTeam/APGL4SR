...
class PCLoss(nn.Module):
    ...
    def forward(self, batch_sample_one, batch_sample_two, intents, intent_ids=None):
        """
        features: 
...
class NCELoss(nn.Module):
    ...
    def forward(self, batch_sample_one, batch_sample_two, intent_ids=None, temp=None):
        if temp == None:
            sim11 = torch.matmul(batch_sample_one, batch_sample_one.T) / self.temperature
            sim22 = torch.matmul(batch_sample_two, batch_sample_two.T) / self.temperature
            sim12 = torch.matmul(batch_sample_one, batch_sample_two.T) / self.temperature
        else:
            sim11 = torch.matmul(batch_sample_one, batch_sample_one.T) / temp
            sim22 = torch.matmul(batch_sample_two, batch_sample_two.T) / temp
            sim12 = torch.matmul(batch_sample_one, batch_sample_two.T) / temp
        d = sim12.shape[-1]
...
class SelfAttention(nn.Module):
    def __init__(self, args):
        ...
        self.dense = nn.Linear(args.hidden_size * 4, args.hidden_size)
        self.LayerNorm = LayerNorm(args.hidden_size, eps=1e-12)
    ...
    def forward(self, input_tensor, attention_mask, att_bias=None):
...
class Layer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, att_bias=None):
...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, att_bias=None, noise=False):
        all_encoder_layers = []
